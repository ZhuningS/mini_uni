<template>
    <!-- pages/wxCase/laohuji/index.wxml -->
    <view class="cont">
        <!-- 底部抽奖 -->
        <view class="luck-draw" v-if="luckDrawID == 1">
            <image class="draw-img" :src="bg"></image>
            <image class="show-draw" v-if="drawId == 1" src="https://i.postimg.cc/qRRLS16Q/susu0.jpg" @tap="getAnmiation"></image>
            <image class="show-draw" v-else src="https://i.postimg.cc/pXDp6RXq/susu3.jpg"></image>
            <view class="content">
                <view class="line">
                    <image :src="tab1.url" :animation="animationData1" @transitionend="getOpenAnimation"></image>
                    <image class="default-img" v-if="!showDefault" :src="tab1.url"></image>
                </view>
                <view class="line">
                    <image :src="tab2.url" :animation="animationData2" @transitionend="getOpenAnimation"></image>
                    <image class="default-img" v-if="!showDefault" :src="tab2.url"></image>
                </view>
                <view class="line">
                    <image :src="tab3.url" :animation="animationData3" @transitionend="getOpenAnimation"></image>
                    <image class="default-img" v-if="!showDefault" :src="tab3.url"></image>
                </view>
            </view>
        </view>
    </view>
...
export default {
    ...
    methods: {
        ...
        getAnmiation: function () {
            ...
            randomNum = parseInt(randomNum, 10); // 隐藏默认图片
            ...
            var that = this; // 校验最终的游戏结果，如果三个结果值都有值，并且全部一致，视为中奖，不一样，视为未中奖

            var resTime = setInterval(function () {
                if (that.resNum.length === 3) {
                    // 延迟1秒给出提示
                    setTimeout(function () {
                        console.error('最新数据==?', that.tab1); // 第一层奖品

                        var tab1 = that.tab1.id; // 第二层奖品

                        var tab2 = that.tab2.id; // 第三层奖品

                        var tab3 = that.tab3.id;
        ...
        getOpenAnimation: function (line, resNum) {
            ...
            randomTotalTime = parseInt(randomTotalTime, 10); // 随即生成每次循环间隔的时间,500-600之间的随机数
            ...
            let loop = setInterval(function () {
                ...
                function handleSet(page) {
                    if (line === 1) {
                        page.setData({
                            tab1: page.data.itemImgs[num],
                            // 修改显示的图片
                            animationData1: animation.export()
                        });
                    } else if (line === 2) {
                        page.setData({
                            tab2: page.data.itemImgs[num],
                            // 修改显示的图片
                            animationData2: animation.export()
                        });
                    } else if (line === 3) {
                        page.setData({
                            tab3: page.data.itemImgs[num],
                            // 修改显示的图片
