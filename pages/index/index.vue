    ...
    <view>
        ...
        <view @touchstart="tap">
            <view class="head">
                ...
                <view class="star head_name" @tap.stop.prevent="showTip">创作不易</view>
                <view class="head_subname">期待您的关注+star~</view>
            </view>
            <view class="con">
                <block v-for="(item, index) in page_list" :key="item.page_list">
                    <view class="c_item">
                        <view class="flex-row j_b">
                            <view>{{ item.name }}</view>
                            <view class="show_img" @tap.stop.prevent="showMore" :data-index="index">
                                <image src="/static/img/good_cou02.png" class="down_arr" v-if="!item.showmore"></image>
                                <image src="/static/img/clolse_down.png" class="down_arr" v-else></image>
                            </view>
                        </view>
                        <view v-if="item.showmore" class="showMore">
                            <block v-for="(item, index1) in item.list" :key="index1">
                                <view class="i_item" @tap.stop.prevent="toDetail" :data-url="item.url">{{ index + 1 }}.{{ item.name }}</view>
                            </block>
                        </view>
    ...
    methods: {
        ...
        getSeconds(s) {
            ...
            if (sTime > 60) {
                //如果秒数大于60，将秒数转换成整数
                //获取分钟，除以60取整数，得到整数分钟
                mTime = parseInt(sTime / 60); //获取秒数，秒数取佘，得到整数秒数

                sTime = parseInt(sTime % 60); //如果分钟大于60，将分钟转换成小时

                if (mTime > 60) {
                    //获取小时，获取分钟除以60，得到整数小时
                    hTime = parseInt(mTime / 60); //获取小时后取佘的分，获取分钟除以60取佘的分
