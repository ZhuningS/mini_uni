...
export default {
    ...
    onLoad: function (options) {
        this.echartsComponnet = this.$mp.page.selectComponent('#mychart-dom-bar').$vm;
        this.getData('echartsComponnet', 0); //获取数据

        this.echartsComponnetLine = this.$mp.page.selectComponent('#mychart-dom-line').$vm;
        this.getData('echartsComponnetLine', 1); //获取数据

        this.echartsComponnetLine1 = this.$mp.page.selectComponent('#mychart-dom-line1').$vm;
        this.getData('echartsComponnetLine1', 2); //获取数据

        this.echartsComponnetpie = this.$mp.page.selectComponent('#mychart-dom-pie').$vm;
    ...
    methods: {
        ...
        getData(type, action) {
            this[type].init((canvas, width, height, dpr) => {
                ...
                Chart.setOption(this.getOption(action)); // 注意这里一定要返回 chart 实例，否则会影响事件处理等
        ...
        getPie() {
            let option = {
                color: ['#3D75FC', '#3E46CE', '#E45C7E', '#2DB4D1', '#CBAE2E', '#5ECAB9', '#D36640'],
                animation: true,
                //控制动画示否开启
                animationDuration: 5000,
                // 动画的时长，它是以毫秒为单位
