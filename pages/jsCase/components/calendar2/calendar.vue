        ...
        <view v-if="!calShow" :class="(isOpen ? '' : 'calendar-box') + ' ' + (dateShow ? 'active' : '')">
            <view :class="'calendar-wrapper ' + (dateShow ? 'active' : '')">
                <view class="calendar-panel">
                    <view class="checkalllbtn" @tap="checkall">全选</view>
                    ...
                    <block v-for="(weeks, week) in canlender.weeks" :key="week">
                        <view class="calender-body-date-week">
                            <block v-for="(day, index) in weeks" :key="index">
                                <view
                                    v-if="filters.strcontains(lockDay, canlender.year + '-' + filters.formatNumber(day.month) + '-' + filters.formatNumber(day.date))"
                                ...
                                <view
                                    ...
                                    :class="
                                        ...
                                        (mini ? 'mini' : '') +
                                        ' ' +
                                        (canlender.month == day.month && ((nowMonth === day.month && day.date - nowDate >= 0) || day.month - nowMonth > 0) ? '' : 'placeholder') +
                                        ' ' +
                ...
                <view v-if="!readonly" class="packup flex flex-between">
                    <view class="packup-button" @tap="clear">重置</view>
                    <view class="packup-button border-left" @tap="packup">确定</view>
                </view>
            </view>
        </view>
    </view>
...
export default {
    ...
    methods: {
        ...
        getWeek(dateData) {
            ...
            let selectDays = this.selectDays; // 判断当前是 安卓还是ios ，传入不容的日期格式
            ...
            for (let i = 1; i <= new Date(year, month, 0).getDate(); i++) {
                let have = false;
                let checked = false; // for (let j = 0; j < selected.length; j++) {
