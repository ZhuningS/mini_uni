...
function fall(
    ...
    H,
    patternPaths,
    { patternSize = 10, difference = 5, duration = Infinity, G = 0.05, fps = 60, SPEED_LIMIT_X = 1, SPEED_LIMIT_Y = 1, deltaTime = 23, tickCount = 600 } = {}
) {
  ...
  */
    ...
    */
    ...
    Snow.prototype.update = function () {
        ...
        this.sx += this.ax; //当x轴方向的加速度达到 速度上线时 进行反向运动
    ...
    function loop() {
        ...
        if (!duration || duration === Infinity) {
            if (ticker > tickCount) {
                snows.push(new Snow(ctx, Math.random() * W, 0, Math.random() * difference + patternSize));
                ticker %= tickCount;
            }
        } else if (ticker > tickCount && new Date() < new Date(execDate + duration * 1000)) {
            //飘落时间限制
            snows.push(new Snow(ctx, Math.random() * W, 0, Math.random() * difference + patternSize));
            ticker %= tickCount;
        }
