...
export default class Painter {
    ...
    _preProcess(view, notClip) {
        ...
        switch (view.type) {
            case 'text': {
                ...
                this.ctx.font = `${textStyle} ${fontWeight} ${view.css.fontSize.toPx()}px ${view.css.fontFamily ? view.css.fontFamily : 'sans-serif'}`; // 计算行数
                ...
                for (let i = 0; i < textArray.length; ++i) {
                    ...
                    const calLines = Math.ceil(textLength / partWidth); // 取最长的作为 width
            ...
            case 'image': {
                ...
                const ratio = getApp().globalData.systemInfo.pixelRatio ? getApp().globalData.systemInfo.pixelRatio : 2; // 有css却未设置width或height，则默认为auto
    ...
    _drawAbsImage(view) {
        ...
        const { width, height } = this._preProcess(view); // 获得缩放到图片大小级别的裁减框
    ...
    _fillAbsText(view) {
        ...
        if (this.isMoving && JSON.stringify(this.movingCache) !== JSON.stringify({})) {
            ...
            const { lines, lineHeight, textArray, linesArray } = extra; // 如果设置了id，则保留 text 的长度
