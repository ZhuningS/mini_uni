...
export default {
    ...
    methods: {
                ...
                success(res) {
                    ...
                    for (let i = 0; i < len; i++) {
                        let item = {
                            ...
                            //自定义选项
                            left: i * (cardWidth / 2),
                            scale: (3 - Math.abs(that.indexArr[i])) * 0.4,
                            zIndex: 3 - Math.abs(that.indexArr[i]),
                            style: {
                                left: i * (cardWidth / 2),
                                transition: 0,
                                zIndex: 3 - Math.abs(that.indexArr[i]),
                                transform: 'scale(' + (3 - Math.abs(that.indexArr[i])) * 0.4 + ')',
                                opacity: 3 - Math.abs(that.indexArr[i]) != 0 ? 1 : 0
                            },
                            backimage: ''
                        };
                        item.option = i;
        ...
        touchmove(e) {
            ...
            let move = pageX - _this.startPageX; //正数，向右滑动；负数，向左滑动
        ...
        touchend(e) {
            ...
            if (list[2].style.left >= list[3].left || list[3].style.left <= list[2].left) {
                ...
                for (let i = 0; i < len; i++) {
                    ...
                    if (3 - Math.abs(_this.indexArr[i]) != 0) {
                        list[i].style.opacity = 1;
                    } else {
                        list[i].style.opacity = 0;
                    }
