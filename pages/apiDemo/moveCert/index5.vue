<template>
    <view>
        <!-- pages/apiDemo/moveCert/index5.wxml -->
        <view class="canvas">
            <canvas type="2d" id="myCanvas" @tap="reset"></canvas>
        </view>
        <view class="text">验证码：{{ code }}</view>
    </view>
</template>
            ...
            query
                ...
                .exec((res) => {
                    ...
                    for (var i = 0; i < that.data.count; i++) {
                        ...
                        var deg = that.randomNum(-30, 30); //修改坐标原点和旋转角度

                        ctx.translate(x, y);
                        ctx.rotate((deg * Math.PI) / 180);
                        ctx.fillText(txt, 5, 0);
                        text = text + txt;
                        console.log(text); //恢复坐标原点和旋转角度
