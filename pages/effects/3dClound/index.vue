<template>
    <!-- pages/effects/3dClound/index.wxml -->
    <view class="tagBall">
        <view
            class="tag"
            :style="'opacity:' + item.o + ';z-index:' + item.z + ';left: ' + item.x + 'rpx;top: ' + item.y + 'rpx;transform: scale(' + item.s + ');color:' + item.color"
            v-for="(item, index) in tagEle"
            :key="index"
        >
            {{ item.title }}
        </view>
    ...
    methods: {
        innit() {
            ...
            for (var i = 0; i < tagEle.length; i++) {
                ...
                var k = (2 * (i + 1) - 1) / tagEle.length - 1; //计算按圆形旋转

                var a = Math.acos(k);
                var b = a * Math.sqrt(tagEle.length * Math.PI); //计算元素x，y坐标

                var numx = 120 * Math.sin(a) * Math.cos(b);
                var numy = 120 * Math.sin(a) * Math.sin(b);
                var numz = 220 * Math.cos(a); // console.log(numo)
                //计算元素缩放大小
