        ...
        <view :class="'lxy-nav-bar__inner ' + (ios ? 'ios' : 'android')" :style="navigationbarinnerStyle + displayStyle">
            <view class="lxy-nav-bar__left" :style="navBarLeft">
                <block v-if="back && !home">
                    <view @tap="backFun" :class="'lxy-nav-bar__button lxy-nav-bar__btn_goback ' + iconTheme"></view>
                </block>
                <block v-if="!back && home">
                    <view @tap="homeFun" :class="'lxy-nav-bar__button lxy-nav-bar__btn_gohome ' + iconTheme"></view>
                </block>
                <block v-else-if="back && home">
                    <view :class="'lxy-nav-bar__buttons ' + (ios ? 'ios' : 'android')" v-if="home">
                        <view @tap="backFun" :class="'lxy-nav-bar__button lxy-nav-bar__btn_goback ' + iconTheme" v-if="back"></view>
                        <view @tap="homeFun" :class="'lxy-nav-bar__button lxy-nav-bar__btn_gohome ' + iconTheme"></view>
                    </view>
                </block>
                <block v-else>
...
export default {
  ...
  show: {
    ...
    onPageShow: function () {
        if (getApp().globalData.globalSystemInfo.ios) {
            this.getSystemInfo();
            this.setStyle(); //设置样式1
        }
    },
    ...
    methods: {
        setStyle: function (life) {
            const { statusBarHeight, navBarHeight, capsulePosition, navBarExtendHeight, ios, windowWidth } = getApp().globalData.globalSystemInfo;
        ...
        getSystemInfo() {
            ...
            if (app.globalData.globalSystemInfo && !app.globalData.globalSystemInfo.ios) {
                return app.globalData.globalSystemInfo;
            } else {
                ...
                app.globalData.globalSystemInfo = systemInfo; //将信息保存到全局变量中,后边再用就不用重新异步获取了
